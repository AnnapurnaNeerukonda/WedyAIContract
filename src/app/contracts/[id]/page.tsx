  ...
  return (
    <main>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">Contract with {contract.clientName}</h1>
        <button onClick={() => router.push("/contracts")} className="text-sm underline">Back to list</button>
      </div>
      <p className="text-sm text-gray-600 mb-2">
        {contract.eventDate?.slice(0, 10)} — {contract.eventVenue} — <span className="uppercase">{contract.status}</span>
      </p>
      ...
      <section className="border rounded-lg p-4 mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          {contract.status === "draft" && editing ? (
            <div className="space-x-2">
              <button onClick={saveEdits} className="text-sm underline">Save</button>
              <button onClick={() => { setEditing(false); setContentHtml(contract.contentHtml); setEditClientName(contract.clientName); setEditEventDate(contract.eventDate?.slice(0, 10)); setEditEventVenue(contract.eventVenue); setEditServicePackage(contract.servicePackage); }} className="text-sm underline">Cancel</button>
            </div>
          ) : (
            contract.status === "draft" && !editing && (
        ...
        {editing ? (
          <div className="space-y-4">
            ...
            />
            <textarea
              className="w-full border rounded-md px-3 py-2 min-h-[120px] focus:outline-none focus:ring-2 focus:ring-black/10"
              value={contentHtml.replace(/<[^>]+>/g, "")}
              onChange={e => setContentHtml(`<p>${e.target.value.replace(/\n/g, "</p><p>")}</p>`)}
        ...
        {contract.status === "signed" ? (
          <div className="space-y-2">
            ...
            {contract.signature?.type === "drawn" ? (
              ...
              <p className="font-[cursive] text-3xl">{contract.signature?.data}</p>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-sm mb-1">Draw signature</p>
              <SignatureCanvas ref={sigRef as any} penColor="black" canvasProps={{ className: "border w-full h-[200px] rounded" }} />
              <div className="mt-2 space-x-2">
                <button onClick={() => sigRef.current?.clear()} className="text-sm underline">Clear</button>
                <button onClick={signDrawn} className="text-sm underline">Sign (drawn)</button>
              </div>
            </div>
            <div>
              <p className="text-sm mb-1">Type name</p>
              <input className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black/10" value={typedName} onChange={(e) => setTypedName(e.target.value)} placeholder="Your full name" />
              <button onClick={signTyped} className="mt-2 text-sm underline">Sign (typed)</button>
            </div>
          </div>
        )}
      </section>
